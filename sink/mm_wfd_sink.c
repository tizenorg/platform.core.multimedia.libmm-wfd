...
#if 0
int mm_wfd_sink_set_HDCP(MMHandleType wfd_sink, void *hdcp, MMWfdSinkHDCPVersion hdcp_version, int hdcp_port)
{
	char *err_name = NULL;
	int result = MM_ERROR_NONE;
	...
	result = mm_wfd_sink_set_attribute(wfd_sink, &err_name, "hdcp_handle", hdcp, sizeof(hdcp),  NULL);
	if (result  != MM_ERROR_NONE)
	{
		debug_error("fail to set HDCP handle.\n");
		return result;
	}

	result = mm_wfd_sink_set_attribute(wfd_sink, &err_name, "hdcp_version", hdcp_version, NULL);
	if (result  != MM_ERROR_NONE)
	{
		debug_error("fail to set HDCP verison.\n");
		return result;
	}

	result = mm_wfd_sink_set_attribute(wfd_sink, &err_name, "hdcp_port", hdcp_port, NULL);
	if (result  != MM_ERROR_NONE)
	{
		debug_error("fail to set HDCP socket port.\n");
		return result;
	}
