...
#if 0
/**
 * This function sets the hdcp version and the number of tcp port\n
 *
 * @param	wfd_sink		[in]	Handle of wi-fi display sink
 * @param	hdcp_handle		[in]	HDCP handle
 * @param	hdcp_version		[in]	HDCP verision
 * @param	hdcp_port		[in]	HDCP TCP port number
 *
 * @return	This function returns zero on success, or negative value with error code.
 * @pre		wi-fi display state should be MM_WFD_SINK_STATE_NULL. \n
 *
 * @par Example
 * @code
gint g_hdcp_version=MM_WFD_SINK_HDCP_v2_1;
gint g_hdcp_port=1234;

if (mm_wfd_sink_set_HDCP(g_wfd_sink, g_hdcp_handle, g_hdcp_version, g_hdcp_port) != MM_ERROR_NONE)
{
	debug_error("failed to set hdcp.\n");
}
 * @endcode
 */
 int mm_wfd_sink_set_HDCP(MMHandleType wfd_sink, void *hdcp_handle, MMWfdSinkHDCPVersion hdcp_version, int hdcp_port);
#endif
