...
mm_wfd_sink_ini_load (mm_wfd_sink_ini_t* ini)
{
	...
	if ( dict ) /* if dict is available */
	{
		...
		MM_WFD_SINK_INI_GET_STRING( dict, ini->name_of_audio_splitter, "pipeline:audio splitter element", DEFAULT_NAME_OF_AUDIO_SPLITTER );
	...
	else /* if dict is not available just fill the structure with default value */
	{
		...
		strncpy( ini->name_of_audio_splitter, DEFAULT_NAME_OF_AUDIO_SPLITTER, WFD_SINK_INI_MAX_STRLEN - 1 );
	...
	debug_log("name_of_audio_splitter : %s\n", ini->name_of_audio_splitter);
